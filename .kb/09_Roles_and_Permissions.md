...
12 system roles · 139 permissions.
...
| --- | --- | --- | --- | --- |
| `SUPER_ADMIN` | Super Admin | PLATFORM | 139 / 139 | Platform operator. Seeded directly into the database, never created via the UI. |
| `ORG_OWNER` | Organization Owner | ORGANIZATION | 123 / 139 | Registered the clinic. Full control including subscription and billing. |
| `ORG_ADMIN` | Organization Admin | ORGANIZATION | 119 / 139 | Manages every branch, but cannot change the subscription or delete the org. |
| `BRANCH_ADMIN` | Branch Admin | BRANCH | 86 / 139 | Runs one or more branches. Which branches is decided per assignment in membership_roles, not here. |
| `DOCTOR` | Doctor | BRANCH | 37 / 139 | Consults patients. Scoped to the branches they practise at. |
| `NURSE` | Nurse | BRANCH | 20 / 139 | Records vitals, assists the encounter, manages the queue. |
...
| `organization.onboarding.read` | Y | Y | Y |  |  |  |  |  |  |  |  |  |
| `organization.onboarding.write` | Y | Y | Y |  |  |  |  |  |  |  |  |  |
