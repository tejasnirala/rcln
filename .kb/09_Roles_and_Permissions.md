...
12 system roles · 134 permissions.
...
| --- | --- | --- | --- | --- |
| `SUPER_ADMIN` | Super Admin | PLATFORM | 134 / 134 | Platform operator. Seeded directly into the database, never created via the UI. |
| `ORG_OWNER` | Organization Owner | ORGANIZATION | 118 / 134 | Registered the clinic. Full control including subscription and billing. |
| `ORG_ADMIN` | Organization Admin | ORGANIZATION | 114 / 134 | Manages every branch, but cannot change the subscription or delete the org. |
| `BRANCH_ADMIN` | Branch Admin | BRANCH | 83 / 134 | Runs one or more branches. Which branches is decided per assignment in membership_roles, not here. |
| `DOCTOR` | Doctor | BRANCH | 37 / 134 | Consults patients. Scoped to the branches they practise at. |
| `NURSE` | Nurse | BRANCH | 20 / 134 | Records vitals, assists the encounter, manages the queue. |
| `RECEPTIONIST` | Receptionist / Front Desk | BRANCH | 24 / 134 | Registers patients, books appointments, collects payment. No clinical access. |
| `LAB_ASSISTANT` | Lab Assistant | BRANCH | 6 / 134 | Collects samples and enters results. Cannot verify or release a report. |
| `LAB_MANAGER` | Lab Manager | BRANCH | 18 / 134 | Verifies results and releases reports. Separation of duty from the assistant. |
| `PHARMACIST` | Pharmacist | BRANCH | 38 / 134 | Dispenses against prescriptions and manages stock. |
| `ACCOUNTANT` | Accountant | ORGANIZATION | 25 / 134 | Billing and revenue. Reads patient identity only, never clinical notes. |
| `PATIENT` | Patient | ORGANIZATION | 11 / 134 | Portal access to their own records only. Row filtering is by patient_id, not by this role. |
