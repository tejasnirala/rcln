...
12 system roles · 112 permissions.
...
| --- | --- | --- | --- | --- |
| `SUPER_ADMIN` | Super Admin | PLATFORM | 112 / 112 | Platform operator. Seeded directly into the database, never created via the UI. |
| `ORG_OWNER` | Organization Owner | ORGANIZATION | 100 / 112 | Registered the clinic. Full control including subscription and billing. |
| `ORG_ADMIN` | Organization Admin | ORGANIZATION | 96 / 112 | Manages every branch, but cannot change the subscription or delete the org. |
| `BRANCH_ADMIN` | Branch Admin | BRANCH | 70 / 112 | Runs one or more branches. Which branches is decided per assignment in membership_roles, not here. |
| `DOCTOR` | Doctor | BRANCH | 31 / 112 | Consults patients. Scoped to the branches they practise at. |
| `NURSE` | Nurse | BRANCH | 17 / 112 | Records vitals, assists the encounter, manages the queue. |
| `RECEPTIONIST` | Receptionist / Front Desk | BRANCH | 22 / 112 | Registers patients, books appointments, collects payment. No clinical access. |
| `LAB_ASSISTANT` | Lab Assistant | BRANCH | 6 / 112 | Collects samples and enters results. Cannot verify or release a report. |
| `LAB_MANAGER` | Lab Manager | BRANCH | 15 / 112 | Verifies results and releases reports. Separation of duty from the assistant. |
| `PHARMACIST` | Pharmacist | BRANCH | 27 / 112 | Dispenses against prescriptions and manages stock. |
| `ACCOUNTANT` | Accountant | ORGANIZATION | 22 / 112 | Billing and revenue. Reads patient identity only, never clinical notes. |
| `PATIENT` | Patient | ORGANIZATION | 11 / 112 | Portal access to their own records only. Row filtering is by patient_id, not by this role. |
