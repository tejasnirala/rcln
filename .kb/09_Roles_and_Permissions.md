...
12 system roles · 96 permissions.
...
| --- | --- | --- | --- | --- |
| `SUPER_ADMIN` | Super Admin | PLATFORM | 96 / 96 | Platform operator. Seeded directly into the database, never created via the UI. |
| `ORG_OWNER` | Organization Owner | ORGANIZATION | 88 / 96 | Registered the clinic. Full control including subscription and billing. |
| `ORG_ADMIN` | Organization Admin | ORGANIZATION | 84 / 96 | Manages every branch, but cannot change the subscription or delete the org. |
| `BRANCH_ADMIN` | Branch Admin | BRANCH | 58 / 96 | Runs one or more branches. Which branches is decided per assignment in membership_roles, not here. |
| `DOCTOR` | Doctor | BRANCH | 29 / 96 | Consults patients. Scoped to the branches they practise at. |
| `NURSE` | Nurse | BRANCH | 14 / 96 | Records vitals, assists the encounter, manages the queue. |
| `RECEPTIONIST` | Receptionist / Front Desk | BRANCH | 17 / 96 | Registers patients, books appointments, collects payment. No clinical access. |
| `LAB_ASSISTANT` | Lab Assistant | BRANCH | 6 / 96 | Collects samples and enters results. Cannot verify or release a report. |
| `LAB_MANAGER` | Lab Manager | BRANCH | 12 / 96 | Verifies results and releases reports. Separation of duty from the assistant. |
| `PHARMACIST` | Pharmacist | BRANCH | 21 / 96 | Dispenses against prescriptions and manages stock. |
| `ACCOUNTANT` | Accountant | ORGANIZATION | 16 / 96 | Billing and revenue. Reads patient identity only, never clinical notes. |
| `PATIENT` | Patient | ORGANIZATION | 11 / 96 | Portal access to their own records only. Row filtering is by patient_id, not by this role. |
...
| permission | SUPER ADMIN | ORG OWNER | ORG ADMIN | BRANCH ADMIN | DOCTOR | NURSE | RECEPTIONIST | LAB ASSISTANT | LAB MANAGER | PHARMACIST | ACCOUNTANT | PATIENT |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `clinical.encounter.close` | Y | Y | Y |  | Y |  |  |  |  |  |  |  |
| `clinical.encounter.create` | Y | Y | Y |  | Y |  |  |  |  |  |  |  |
| `clinical.encounter.read` | Y | Y | Y | Y | Y | Y |  |  |  |  |  |  |
| `clinical.master.manage` | Y | Y | Y |  |  |  |  |  |  |  |  |  |
| `clinical.prescription.create` | Y | Y | Y |  | Y |  |  |  |  |  |  |  |
| `clinical.prescription.read` | Y | Y | Y | Y | Y | Y |  |  |  | Y |  | Y |
| `clinical.prescription.sign` | Y | Y | Y |  | Y |  |  |  |  |  |  |  |
