...
12 system roles · 122 permissions.
...
| --- | --- | --- | --- | --- |
| `SUPER_ADMIN` | Super Admin | PLATFORM | 122 / 122 | Platform operator. Seeded directly into the database, never created via the UI. |
| `ORG_OWNER` | Organization Owner | ORGANIZATION | 107 / 122 | Registered the clinic. Full control including subscription and billing. |
| `ORG_ADMIN` | Organization Admin | ORGANIZATION | 103 / 122 | Manages every branch, but cannot change the subscription or delete the org. |
| `BRANCH_ADMIN` | Branch Admin | BRANCH | 75 / 122 | Runs one or more branches. Which branches is decided per assignment in membership_roles, not here. |
| `DOCTOR` | Doctor | BRANCH | 34 / 122 | Consults patients. Scoped to the branches they practise at. |
| `NURSE` | Nurse | BRANCH | 17 / 122 | Records vitals, assists the encounter, manages the queue. |
| `RECEPTIONIST` | Receptionist / Front Desk | BRANCH | 22 / 122 | Registers patients, books appointments, collects payment. No clinical access. |
| `LAB_ASSISTANT` | Lab Assistant | BRANCH | 6 / 122 | Collects samples and enters results. Cannot verify or release a report. |
| `LAB_MANAGER` | Lab Manager | BRANCH | 18 / 122 | Verifies results and releases reports. Separation of duty from the assistant. |
| `PHARMACIST` | Pharmacist | BRANCH | 31 / 122 | Dispenses against prescriptions and manages stock. |
| `ACCOUNTANT` | Accountant | ORGANIZATION | 22 / 122 | Billing and revenue. Reads patient identity only, never clinical notes. |
| `PATIENT` | Patient | ORGANIZATION | 11 / 122 | Portal access to their own records only. Row filtering is by patient_id, not by this role. |
