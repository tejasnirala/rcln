...
Declared at `packages/db/prisma/schema/invoicing.prisma:181`.
...
| `kind` | `InvoiceKind` | `kind InvoiceKind @default(INVOICE)` |
| `creditedInvoiceId` | `String?` | `creditedInvoiceId String? @map("credited_invoice_id") @db.Uuid` |
...
| `creditedInvoice` | [`Invoice`](Invoice.md) | `creditedInvoice Invoice? @relation("InvoiceCreditNote", fields: [organizationId, creditedInvoiceId], references: [organizationId, id], onDelete: Restrict, onUpdate: NoAction)` |
| `creditNotes` | [`Invoice`](Invoice.md) | `creditNotes Invoice[] @relation("InvoiceCreditNote")` |
