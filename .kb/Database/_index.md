...
121 models · 95 enums · 94 tenant-scoped.
...
| `Appointment` | `organization: Organization` `branch: Branch` `patient: Patient` `registration: PatientRegistration` `doctorProfile: DoctorProfile` `booker: User?` `canceller: User?` `statusHistory: AppointmentStatusHistory[]` `vitals: AppointmentVital[]` `invoices: Invoice[]` `reschedules: AppointmentReschedule[]` `parent: Appointment?` `followUps: Appointment[]` `clinicalEpisode: ClinicalEpisode` `recommendationsMade: EncounterFollowUpRecommendation[]` `recommendationsFulfilled: EncounterFollowUpRecommendation[]` |
...
| `ClinicalEpisode` | `organization: Organization` `patient: Patient` `primarySpecialty: Specialty?` `opener: User?` `closer: User?` `appointments: Appointment[]` |
...
| `ConsultationTemplate` | `organization: Organization?` `careContext: Specialty` `specialty: Specialty?` `versions: ConsultationTemplateVersion[]` |
| `ConsultationTemplateVersion` | `organization: Organization?` `template: ConsultationTemplate?` `publisher: User?` |
...
| `DoctorProfile` | `organization: Organization` `user: User` `specialties: DoctorSpecialty[]` `qualifications: DoctorQualification[]` `branchSettings: DoctorBranchSetting[]` `compensation: DoctorCompensation?` `fees: FeeScheduleEntry[]` `invoices: Invoice[]` `schedules: DoctorSchedule[]` `exceptions: DoctorScheduleException[]` `appointments: Appointment[]` |
...
| `EncounterFollowUpRecommendation` | `organization: Organization` `branch: Branch` `patient: Patient` `appointment: Appointment` `fulfilledBy: Appointment?` `canceller: User?` |
...
| `NumberSequenceType` | `UHID` `MRN` `APPOINTMENT` `QUEUE_TOKEN` `EMPLOYEE` `INVOICE` `STOCK_TRANSFER` `PURCHASE_REQUISITION` `PURCHASE_ORDER` `GOODS_RECEIPT` `PURCHASE_RETURN` `DISPENSE` `CLINICAL_EPISODE` |
