...
| **Degraded** | GB, AE (federal)   | legislation.gov.uk returned `202` on every attempt, HTML and XML alike, and rendered empty through the fetch proxy. **PI-14 needs an access route found before it starts** — the published PDFs are one candidate. ⚠️ **PI-17 found the UAE's FEDERAL sources in the same state**: `uaelegislation.gov.ae` `403`, `mohap.gov.ae` connection reset. Its two emirate regulators are reachable and the pack shipped sub-nationally only |
