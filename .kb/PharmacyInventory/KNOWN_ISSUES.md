...
| 2   | ~~**`pnpm test` OOMs the api container outright.**~~ **CLOSED IN PI-24, AND IT WAS TWO FAULTS WEARING ONE ENTRY.** (a) The ceiling is the container's, so the fix is to RECYCLE the worker, not to enlarge the heap: `workerIdleMemoryLimit: '1200MB'` in `apps/api/jest.config.ts`. One process was accumulating 102 module registries under `maxWorkers: 1` and nothing evicted them; raising `--max-old-space-size` reached the same wall in a state the kernel answers with SIGKILL. (b) The hang that needed `--forceExit` was the memoised BullMQ producer from `issueInvoice` — `maxRetriesPerRequest: null`, so it blocks on `BRPOPLPUSH` and never idles out — which only four suites closed. `--detectOpenHandles` reported NOTHING for it, because it is opened lazily inside a service rather than by test code. `tests/setup-after-env.ts` closes it after every suite. **`pnpm test` now runs unsharded: 103 suites, 2,191 cases, 77s, exit 0, no `--forceExit`.** ⚠️ Import it INSIDE the hook — a top-level import there freezes `config` before `storage-path.test.ts` can set its variable, which broke four cases the first time. | —                                                                                      | —                                                                                                                                                                                                                  |
...
| 7   | ~~**`tax-registration-coverage.test.ts` is flaky in a full run.**~~ **CLOSED IN PI-24, AND IT WAS NEVER A TEST PROBLEM.** A branch that has stated no coverage takes the jurisdiction fallback and was offered EVERY live registration in its state — **including one the clinic had explicitly assigned to another branch**. `registrationFor` then chose between two rows sharing an `effectiveFrom` by whichever order Postgres returned them, so Gaya's dedicated GSTIN printed on about half of Patna's invoices and the tax was filed against a registration that does not cover the branch that collected it. India has permitted a second GSTIN per state since 2019, so this is the ordinary shape. Fixed in two layers: `tax.service.ts` no longer offers a registration somebody has claimed, and `packages/tax/src/selection.ts` breaks the tie on `registrationNumber` — the determinism its own header had promised and delivered only when the dates differed.                                                                                                                                                                        | —                                                                                      | —                                                                                                                                                                                                                  |
...
### ✅ KI-6 — Notification delivery — CLOSED IN PI-24 for email, still open for SMS

**Email was never the stub the summary suggested** — `smtpSender` has delivered
since Mailpit was wired in. What was missing was a CONSUMER: `QUEUE.NOTIFICATIONS`
existed and its worker handler logged `"processor not implemented yet"`, so every
pharmacy event reached a log line (KNOWN_ISSUES #26).

Closed by moving the sender into **`@rcln/notifications`** — the API could not
share it because it read that app's `config` and `logger` directly, and copying
it into the worker is what this entry forbids by name. The worker now sends on
order confirmation, dispatch and failed delivery, and on stock expiring within 30
days.

⚠️ **SMS IS STILL A LOGGING STUB, AND IT IS AN EXTERNAL BLOCKER RATHER THAN CODE.**
TRAI DLT registration — entity, header, and one approval per template — is a 1–2
week process, and an unregistered SMS in India is dropped by the carrier rather
than failing loudly. When it clears: add a sender satisfying `NotificationSender`
and change one line in `createSender`. **Then delete the master verification code**
(`sender.ts` says why).

⚠️ **AND NO BODY MAY CARRY PHI.** The delivery emails send a reference, a status
and a link — no patient name, no medicine, no clinic name. They go to an external
relay, are stored in plain text at the far end, and are read on lock screens.
"Your metformin has shipped" names a condition. Everything else is behind the
sign-in. Changing that is a PHI decision, not a copy change.
...
### ✅ KI-9 — Product catalogue data has no source — MITIGATED IN PI-24 (an importer, not data)

**The platform still ships no product data, and this session did not invent
any** — wrong strengths or schedules in a clinical system are dangerous, and a
plausible-looking formulary nobody sourced is worse than an empty one.

What changed is the COST of the clinic's own data. `POST /v1/products/import`
takes a spreadsheet's rows, names units, categories and manufacturers by their
CODES (a spreadsheet has no uuids), reports **every** bad row rather than the
first, and writes nothing unless the whole file is clean. `dryRun` is part of the
contract rather than a convenience, and the screen at `/products/import` will not
let anybody import before checking.

⚠️ **Prices and tax classifications are deliberately not in it.** A price belongs
to a branch and a currency, a tax classification to a country and a jurisdiction;
one spreadsheet column cannot honestly mean both. They are separate imports, and
for an Indian clinic the HSN/GST half is the obvious next one.

### KI-9 (original) — Product catalogue data has no source
...
| #   | Item                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Severity                                                          | Mitigation                                                                                                                                                                          |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 23  | ~~**An abandoned order keeps its `CONFIRMED` status after the sweep releases its hold.**~~ **CLOSED IN PI-24 — it needed a new status, and the CHECK constraint is what proved it.** `DRAFT` requires `order_number IS NULL`, so rolling back would have DESTROYED the reference the patient was given; `CANCELLED` requires a reason somebody wrote, and nobody wrote one. `EXPIRED` keeps the number and the confirmation and adds nothing — its CHECK branch is `CONFIRMED`'s verbatim. The reservation sweep runs a second pass per branch (`expireAbandonedOrders`), and packing one refuses with a sentence saying the stock went back. Originally: The reservation sweep works on `expires_at` and knows nothing about `online_orders`, so the order reads as accepted with `heldQuantityBase: 0`. Nothing is unsafe — packing refuses with a 409 naming the hold — but the list is misleading and a clinic cannot filter for it.                          | LOW — visible, not dangerous                                      | A status the sweep can set, or a derived "hold lapsed" flag on the summary computed from the reservations the detail already reads. The latter is a screen change and no migration. |
| 24  | ~~**A recall cannot reach stock held for an order nobody has packed.**~~ **CLOSED IN PI-24, AND HALF OF IT WAS ALREADY STALE.** `RESERVED` has been in `RECALLABLE` since the PI-11 review, so `executeRecall` DOES move the quantity. What survived was the reservation ROW: still ACTIVE, still naming the batch, so packing would claim it. `packOnlineOrder` now refuses a hold whose batch is RECALLED, naming the lot — refused rather than silently re-picked, because which lot replaces it is a decision somebody should make with the recall in front of them. Originally: PI-10 walks `dispense_allocations`, so a PACKED parcel IS traced and un-dispensable stock is un-dispensable. An order that is merely `CONFIRMED` holds its lots in the `RESERVED` bucket, which `executeRecall` does not move — so the parcel could still be made up from a recalled lot after the notice was executed. ⚠️ **The most consequential gap this phase leaves.** | MEDIUM — a recalled lot can reach a patient through an open order | Extend `executeRecall` to release or quarantine `ACTIVE` reservations over the recalled lots, and refuse the pack when a lot is no longer dispensable. PI-22/PI-23 territory.       |
...
| 26  | ~~**No worker, no notification.**~~ **CLOSED IN PI-24 for email.** `QUEUE.NOTIFICATIONS` had a handler that logged "processor not implemented yet"; it now sends on confirm, dispatch and failed delivery, plus a stock-expiring alert to staff. The sender moved to `@rcln/notifications` because the API's copy read that app's `config` and `logger` and the worker could not use it — a second sender is what KI-6 forbids. ⚠️ SMS is still stubbed, blocked on TRAI DLT registration. ⚠️ And no body carries PHI: reference, status and a link, never a name or a medicine. Originally: An accepted order tells the patient nothing, and neither does a shipment. The programme has a notification service; this phase wired none of it, because who is told what about a medicine going to a house is a decision nobody has taken.                                                                                                                          | LOW — a product gap, not a defect                                 | A decision first, then a processor.                                                                                                                                                 |
...
### ✅ `AU-SCHEDULE-S8` REFUSED EVERY SCHEDULE 8 TRANSACTION OUTSIDE VICTORIA — CLOSED IN PI-24, and it had a twin

**Fixed, and the fix is neither option this entry proposed.** Deleting the rule
loses the label the pack wanted; relaxing the parser to accept any obligation-free
rule turns a mistyped parameter into a **fail-open in a controlled-drugs engine**,
which is the one direction this programme must not move in.

`informationalOnly: true` is an **explicit** opt-out from the "imposes no
obligation" refusal. A pack that genuinely only labels says so and is permitted;
a rule that imposes nothing WITHOUT saying so still fails closed, because that is
what a mistyped `registerRequired` looks like. Setting the flag beside a real
obligation is refused as a contradiction, and an informational rule with no
`scheduleName` is refused as empty.

⚠️ **AND THE INSTRUCTION TO CHECK THE OTHER PACKS WAS RIGHT: `SG-SCHEDULE-CD3`
HAD THE IDENTICAL SHAPE AND NOBODY HAD RECORDED IT.** Every Third Schedule supply,
movement, transfer and disposal in Singapore was refused by the rule written to
label it — its statement even reads "which require no register for this schedule".
Found by running the parser over all ten packs rather than by reading them.

⚠️ **BOTH BEHAVIOUR CASES PASSED THE WHOLE TIME.** Asserting that the rule code
appears in the reasons and that no conditions were raised describes an unreadable
rule exactly as well as a permissive one, and `UNDETERMINED` is not `REFUSED`, so
even Singapore's `not.toBe('REFUSED')` held. Both now assert the outcome, and
`apps/api/tests/unit/rule-pack-readable.test.ts` parses every rule in every pack
through the engine's own dispatch so the class cannot ship a third time.

Ireland got its labels back at the same time: `IE-SCHEDULE-CD3` and
`IE-SCHEDULE-CD4A` exist again, marked informational, with the reading of
Regulation 19(1) unchanged.

### ⚠️ The seed directory is not typechecked, and ten regulatory packs live in it

**Found in PI-24, and it is why the two rules above could carry anything.**
`packages/db/tsconfig.json` excludes `prisma`, so nothing under
`prisma/seed/data/` — every country rule pack — was ever type-checked. The new
`rule-pack-readable.test.ts` drags the packs into `apps/api`'s test project and
immediately surfaced a latent error in `regulatory-ie.ts`, which is now fixed.
That gives the packs type coverage as a side effect of a test, which is thinner
than it should be: `packages/db` should typecheck its own seed directory.

### ~~⚠️ `AU-SCHEDULE-S8` REFUSES EVERY SCHEDULE 8 TRANSACTION OUTSIDE VICTORIA — a PI-15 defect, found in PI-18~~ (the original entry, kept for the diagnosis)
...
### ✅ PI-24's review sweep — what it found, and what is still open

**Four reviewers over `git diff c0f3adc..HEAD`, 267 files. Everything below was
verified by RUNNING the code, not by reading it.** All fixed except the last.

**Fail-OPEN, engine (all fixed):**

1. **An unrecognised classification permitted.** `needsClassificationButHasNone`
   tested `classification !== null` rather than "is it one this jurisdiction
   names". One typo dropped every classification-keyed rule while the pack's
   unclassified rules still permitted. Live in `IN`, `SG`, `US`, `US-CA`.
2. **An empty parameter list disabled its own limb**, seven ways — including
   `storageLocationKinds: []`, which reopened `AU-SCHEDULE-S8` by a side door.
   `readStringArray` now refuses `[]`.
3. **A future-dated prescription was dispensed** where no validity was configured
   — India and Bangladesh both. The guard now runs unconditionally.
4. **`licenceRequired` with no `licenceType`** was satisfied by any licence at
   all. Refused at the parser now.
5. **`CONTROLLED_SCHEDULE` checked location on `STOCK` only**, while
   `STORAGE_REQUIREMENT` checked `STOCK` and `TRANSFER` — so a pack writing only
   the first form left the transfer path open.

**Fail-OPEN, packs (all fixed):**

6. **Ireland had no `PHARMACIST_AUTHORITY` rule for any controlled schedule** —
   an unlicensed person was refused a Part A POM and permitted morphine.
7. **Singapore had no refill rule for the controlled schedules** — morphine filed
   under the Misuse of Drugs spelling was dispensable without limit.
8. **India's pharmacist gate did not cover `ONLINE_DISPENSE`** — the only
   hand-written transaction list in the pack, so PI-12 silently escaped it.
9. **Eight `STORAGE_REQUIREMENT` rules across seven packs omitted
   `ONLINE_DISPENSE`** — the counter refused what the parcel permitted.

**Correctness (all fixed):** the aging report's `COALESCE(cost, 0)` (uncosted
stock reported as worth zero, and `unvaluedQuantityBase` structurally dead);
`procedure-contribution` sorting and totalling a page-sized ARBITRARY slice, so
"the fifty most profitable procedures" were fifty random ones; two
`{ organizationId: 'desc' }` orderings that put the PLATFORM row first because
Postgres defaults `DESC` to NULLS FIRST; `issued_last_year` on a UTC window
beside an `idle_days` on the branch's.

**Web (all fixed):** a **line-key collision** — pre-filled lines were keyed by
array index while the counter started at 1, so the first added line shared a key
with the second ordered line and every edit hit both, producing a received line
pointing at the wrong product against a real PO line; report `datetime` columns
rendered in UTC; a failed pre-fill fetch rendering an empty form indistinguishable
from "nothing was asked for"; an order line with no quantity dropped in silence;
`patientId` accepted as a URL query param; `defaultWindow` computing the current
month while its comment promised the last full one.

**Security (all fixed):** the two scan reads scoped only by `branchId` and RLS;
`online_order_shipments` with no cross-tenant case; `req.body.reason` unredacted;
`REPORT_PATHS[reportKey]` reading through `Object.prototype`; and a file header
that claimed `slug` was server-bound when it is client-supplied.

### ⚠️ Abu Dhabi's and Dubai's refill ladders are unreachable — OPEN, and deliberately

`AZ-REFILL-CD/SCD` and `DU-REFILL-CD/SCD` carry `maxEndorsedRepeats: 2`, and the
lawful first refill at day 30 is **refused by two rules at once**: `validityDays: 3`
is measured from `issuedOn`, and the three days is the window to PRESENT the
prescription, not to consume it. `expiredAgainst` has no notion of a repeat clock,
and `PresentedPrescription` carries no `lastDispensedOn`.

**Not fixed, on purpose.** The framework half is mechanical; the pack half is a
reading of DOH §5.4.4 / DHA 18.7.4(e) — that a presentation window does not bind a
subsequent repeat — which nobody here can verify against the source, and whose
error direction is PERMITTING a controlled-drug supply. It fails CLOSED today,
which is the safe direction. **This needs a human with the instruments.**

### ✅ `GET /v1/stock/resolve` has its own rate limiter — CLOSED in PI-24 (was #35)

And it needed a BIGGER budget, not a smaller one: `generalLimiter` allows 100
requests per fifteen minutes, which a storekeeper reaches partway through
unpacking one delivery. `scanLimiter` gives it 40 a minute in its own bucket, so
scan traffic also stops exhausting the budget every other screen shares.

### ✅ The seed directory typechecks — CLOSED in PI-24

`packages/db/tsconfig.json` excludes `prisma`, so ten country rule packs were
never type-checked. `tsconfig.seed.json` covers `prisma/**` and `scripts/**` with
`noEmit` and is wired into `pnpm typecheck`; it found four latent errors
immediately.

### ✅ 22 redundant indexes dropped — PI-24's index audit

An index audit over the live schema found every hot table correctly indexed and
leading with `organization_id`, matching the RLS predicate — but 23 non-unique
indexes that are strict PREFIXES of a unique index on the same table, which
Postgres already serves from the wider one. Twenty-two were dropped;
`product_identifiers`' was kept deliberately, because that table is read on every
scan and written when somebody catalogues a product, so the write-cost argument
...
⚠️ **PI-22 HAS NOT BEEN REVIEWED.** Everything below is a gap the phase knows
about and recorded itself, which is a weaker thing than a review finding.

| #   | Item                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Severity                                                                                          | Mitigation                                                                                                                                                                                                                                                                          |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 27  | **`procedure-contribution` cannot include the procedure's own fee, so "contribution" means the margin on MATERIALS.** Nothing in this schema prices one procedure differently from another: `fee_schedule_entries` prices a fee TYPE and `PROCEDURE` is one string for every procedure a clinic performs; `invoices` carries no reference on a `PROCEDURE` source, by the deliberate design PI-8 argued; and `charge_requests.source_type` is CHECKed to `PHARMACY`/`INVENTORY`. So the revenue side is the consumables that reached an invoice. Every field says `consumable`, the response carries `procedureFeeIncluded: false`, the screen prints it above the table, and the registry entry leads with it — because the one thing that must not happen is somebody reading a contribution figure as profit. ⚠️ **The largest honest gap the phase leaves.** | MEDIUM — the figure is correct and answers a narrower question than its name suggests             | A per-procedure rate card: a price against `clinical_master_items` of kind `PROCEDURE`, and a `PROCEDURE` charge-request source. That is a charging-model change, not a reporting one, and it should carry the `PROCEDURE` invoice reference column PI-8 left out at the same time. |
| 28  | **A product with two cost averages in two currencies produces two rows on `consumption-cost` and `procedure-contribution`.** Both group by currency on purpose — the alternative is a row that subtracts dollars from rupees — but the effect on the CONSUMPTION reports is that the same physical material is counted once per currency, so the two rows are each internally consistent and their quantities do not sum to what was used. PI-4 made this shape explicit rather than fixing it; PI-22 inherits it and does not make it worse.                                                                                                                                                                                                                                                                                                                    | LOW — latent: it needs a clinic that genuinely bought one product in two currencies at one branch | An FX policy, which is PI-4's open item. Until there is one, no report can honestly collapse the two.                                                                                                                                                                               |
| 29  | **`heldSince` is null for a `DAMAGED` or `EXPIRED` quantity on the held-stock report.** Only `quarantined_at` and `recalled_at` exist as columns; a quantity in the damaged or expired bucket was put there by a MOVEMENT, and answering "since when" means a `stock_ledger` walk per held lot — which is the query that takes the screen down on a report listing every held lot in the clinic. The honest null shipped instead of the confident guess.                                                                                                                                                                                                                                                                                                                                                                                                         | LOW — a blank cell, and the report says why                                                       | A `held_since` on `stock_balances`, maintained by the same trigger that maintains the quantity. It is a schema change to a table `rcln_app` cannot write, so it belongs with whoever next touches that trigger.                                                                     |
| 30  | **`daysOfCover` on dead stock uses a 365-day rate anchored to `now()`, not to the branch's day.** Every other date comparison in the directory is resolved `AT TIME ZONE b.timezone`; this one is an interval on an instant, because a rate over a year is not sensitive to which side of midnight it starts. Recorded so the inconsistency is a decision rather than an oversight.                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | LOW — cosmetic at worst                                                                           | Push it through the same branch-local expression as the rest if anybody ever reports a discrepancy.                                                                                                                                                                                 |
| 31  | **The report screens have no picker for `productId`, `categoryId`, `supplierId` or `procedureItemId`.** The API honours all four and the page forwards them, so a link from a product screen lands filtered and a URL is shareable — but nothing on the report screen offers them. The same identifier-resolution debt as #25 and #25b, and the same answer.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | LOW                                                                                               | PI-23's resolver gives every screen one picker instead of nine.                                                                                                                                                                                                                     |

---

## PI-23 (Identifier Resolution / Barcode)

⚠️ **PI-23 HAS NOT BEEN REVIEWED.** Everything below is a gap the phase recorded
about itself, which is a weaker thing than a review finding.

| #   | Item                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Severity                                                                               | Mitigation                                                                                                                                                                                                                            |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 32  | **A scan is not checked against open recall notices, which is what #22 named this phase for.** The resolver surfaces the lot at a goods receipt and reports its `status` and `recalledAt` — so a lot ALREADY recalled shows as undispensable — but it does not ask whether the lot NUMBER appears on an open `recalls` notice for a different `batches` row. That is the case #22 describes: the same printed lot arriving again after the notice.                                                                                                                                                                                                                                                                                             | MEDIUM — a clinic that keeps ordering during a recall still re-stocks the recalled run | Matching the lot to `recall_batches` needs `recall.notice.read` on top of this endpoint's two codes, which widens the gate on the hottest read in the programme. The honest place may be the goods-receipt POST rather than the scan. |
| 33  | ~~**"Consultation" on the online-order form is a raw id box.**~~ **CLOSED IN PI-24.** It needed a new endpoint, and the reason nothing existed is the interesting half: `visit-history` answers the same question and returns DIAGNOSES, gated on `clinical.encounter.read`. Pointing a pharmacy dropdown at it would have put a chart on the counter. `GET /v1/online-orders/patients/{id}/consultations` returns date, doctor and item count — nothing clinical — behind the order code. Originally: Half of #25 is paid — the patient field is a search now — but nothing in `@rcln/contracts` lists a patient's consultations, so a picker here needs a new endpoint. The field is optional and its hint now says where the id comes from. | LOW                                                                                    | `GET /encounters?patientId=` with a status filter, then the same picker shape. It is a contract change, not identifier resolution.                                                                                                    |
| 34  | ~~**The procedure picker on `/usage/templates` is capped at 100.**~~ **CLOSED IN PI-24.** `ProcedurePicker` searches the server per keystroke, the same shape as the product and patient pickers; the page now fetches ONE row, purely to decide whether the dictionary is empty. Originally: It reads `clinical-data?kind=PROCEDURE`, not the product catalogue — a different search behind a different permission, and the only capped picker left in the application.                                                                                                                                                                                                                                                                       | LOW                                                                                    | A `searchProcedures` beside `searchProducts` in `lookup-actions.ts`. The component shape is already there.                                                                                                                            |
| 35  | ~~**`GET /stock/resolve` has no limiter of its own**~~ **CLOSED IN PI-24** — `scanLimiter`, and it needed a BIGGER budget than the general one: 100 requests per fifteen minutes is partway through unpacking one delivery. 40 a minute, in its own bucket. Originally:, which `API_ARCHITECTURE.md` asks for by name on the grounds that it becomes the hottest endpoint in the programme. It carries the global limiter and nothing more.                                                                                                                                                                                                                                                                                                    | LOW — latent until a clinic actually scans at volume                                   | Its own bucket in the rate-limit middleware, sized for a counter rather than for a browser.                                                                                                                                           |
| 36  | **The AI table is the healthcare subset, not the GS1 General Specifications.** About forty identifiers out of roughly 150. Anything else stops the parse and is reported — which is the safe failure, not a silent one — but a clinic receiving unusual logistics labelling will see `unparsed` where a fuller table would have read through.                                                                                                                                                                                                                                                                                                                                                                                                  | LOW — the failure is visible and specific                                              | Add the AI to the table in `gs1.ts` with its length. ⚠️ Never widen it by guessing a length: a wrong length is the silent-wrong-lot failure the file is built to prevent.                                                             |
| 37  | ~~**`ProductPicker` is remounted**~~ **CLOSED IN PI-24** — the picker now takes an optional `value` prop, so the three forms own the choice and the subtree survives a scan (keeping search term, results, scroll and focus). Originally: `key={line.productId}` on the goods-receipt, purchase-order and return forms.** The picker owns its own choice — it is a search box, not a controlled select — so a line filled by a SCAN has to remount it with a new `initial`. The side effect is that the search box and its results are cleared when a scan lands on a line somebody was mid-search on.                                                                                                                                         | LOW — the scan is what the operator asked for                                          | A controlled `value`/`onChange` variant of the picker, if this ever irritates anybody. The remount is deliberate and commented at each call site.                                                                                     |

---

## Resolved

Entries kept because deleting them loses the reason the fix exists.

| #   | Item                                                                                                                                                                                                                                                 | Resolved                          | Fix                                                                                                                                                                                                                                                                                                                                                                             |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 25  | **The order form asked a receptionist to type raw UUIDs.** "Patient" and "Consultation" were free-text inputs while every other id on the screen was a `Select`, and the hint named the database's concern rather than the user's.                   | 2026-09-02, PI-23                 | `PatientPicker` searches by name, phone or UHID at the branch, through `searchPatients` in `lookup-actions.ts`. ⚠️ **Half-resolved only:** "Consultation" is still an id box and is now #33.                                                                                                                                                                                    |
| 25b | **Every product picker was capped at 100 or 200 rows.** Eleven screens fetched a page of the catalogue at render and filtered it in a `<select>`; a clinic with three thousand products had a picker that could not reach most of its own catalogue. | 2026-09-02, PI-23                 | `ProductPicker` searches on the server through `searchProducts`, with the same `trackingMode` / `isStockItem` filters applied to the SEARCH rather than to a fetched page. `PRODUCT_CAP` and `PICKER_LIMIT` are gone from every product picker in the application. The lot pickers were the same debt and went with them — `lotsForProduct` asks for one product at one branch. |
| 31  | **The report screens had no picker for `productId`, `categoryId`, `supplierId` or `procedureItemId`.**                                                                                                                                               | 2026-09-02, PI-23 — **partially** | The component the report screens need now exists and is used on eleven other screens; wiring it into `/reports/[reportKey]` is a screen change with no API work behind it. ⚠️ **Not done in PI-23** — recorded here rather than left in the PI-22 table, because the blocker it named is gone.                                                                                  |
