...
| 2   | **`pnpm test` OOMs the api container outright.** ⚠️ **PI-22 CORRECTED THE DIAGNOSIS: THE CEILING IS THE CONTAINER, NOT V8.** This entry's mitigation assumed a heap limit; raising `--max-old-space-size` to 4096 gets the process **SIGKILLed** against `mem_limit: 3g` rather than throwing, while a THIRD of the suite at 2400 fits. So the fix is the compose limit or `workerIdleMemoryLimit`, and not the Node flag alone. Original text: PI-3 recorded this for `pnpm validate` when turbo ran tasks in parallel; adding PI-4's two suites tipped the api project itself over the 3 GB heap. `pnpm exec turbo run typecheck\|lint` still pass at `--concurrency=1`, and the api suite has to be run in slices by path.                                                                                                                            | MEDIUM — it makes "run the tests once at the end" impractical                          | Raise the container's memory limit, or set `--maxWorkers=1` plus `--workerIdleMemoryLimit` in `apps/api/jest.config.ts`. It is masking real failures behind a crash, which is the part that matters.               |
