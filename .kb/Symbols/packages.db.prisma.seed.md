...
| --- | --- | --- | --- |
| `day` <sub>local</sub> | `(value: string): Date` | `packages/db/prisma/seed/regulatory-packs.ts:113` | A `@db.Date` column takes a day, in the jurisdiction's own reckoning. |
...
| `seedRegulatoryPacks` | `(): Promise<void>` | `packages/db/prisma/seed/regulatory-packs.ts:427` |  |
...
| `PACKS` <sub>local</sub> | `: readonly PackSeed[]` | `packages/db/prisma/seed/regulatory-packs.ts:148` |  |
...
| name | signature | at | notes |
| --- | --- | --- | --- |
| `PackSeed` <sub>local</sub> | `{ countryCode, regionCode, countryName, authorities, sources, rules, packAuthorityCode, version, name, description, effectiveFrom }` | `packages/db/prisma/seed/regulatory-packs.ts:117` |  |
