...
| `moduleOf` | `(code: PermissionCode): string` | `packages/permissions/src/codes.ts:646` | Split `pharmacy.dispense.create` into its module for grouping in the UI. |
...
| name | signature | at | notes |
| --- | --- | --- | --- |
| `ALL_PERMISSIONS` | `Object.values(PERMISSIONS) as PermissionCode[]` | `packages/permissions/src/codes.ts:643` |  |
...
| `REGULATORY_SIGN_OFF` <sub>local</sub> | `: PermissionCode[]` | `packages/permissions/src/roles.ts:88` |  |
| `SYSTEM_ROLE_DEFINITIONS` | `: SystemRoleDefinition[]` | `packages/permissions/src/roles.ts:112` |  |
...
| `PermissionCode` | `(typeof PERMISSIONS)[keyof typeof PERMISSIONS]` | `packages/permissions/src/codes.ts:641` |  |
