...
| `moduleOf` | `(code: PermissionCode): string` | `packages/permissions/src/codes.ts:497` | Split `pharmacy.dispense.create` into its module for grouping in the UI. |
...
| name | signature | at | notes |
| --- | --- | --- | --- |
| `ALL_PERMISSIONS` | `Object.values(PERMISSIONS) as PermissionCode[]` | `packages/permissions/src/codes.ts:494` |  |
...
| `PERMISSIONS` | `{ // -- platform (super admin only) ------------------------------------------- PLATFORM_…` | `packages/permissions/src/codes.ts:31` |  |
| `SYSTEM_ROLE_DEFINITIONS` | `: SystemRoleDefinition[]` | `packages/permissions/src/roles.ts:66` |  |
...
| name | signature | at | notes |
| --- | --- | --- | --- |
| `Module` | `(typeof MODULES)[number]` | `packages/permissions/src/codes.ts:29` |  |
| `OverrideEffect` | `'GRANT' \| 'DENY'` | `packages/permissions/src/resolver.ts:18` |  |
| `PermissionCode` | `(typeof PERMISSIONS)[keyof typeof PERMISSIONS]` | `packages/permissions/src/codes.ts:492` |  |
