...
| `authorsClinicalNotes` <sub>local</sub> | `(p: PermissionCode): boolean` | `packages/permissions/src/roles.ts:64` |  |
...
| `maintainsPlatformLaw` <sub>local</sub> | `(p: PermissionCode): boolean` | `packages/permissions/src/roles.ts:109` |  |
| `moduleOf` | `(code: PermissionCode): string` | `packages/permissions/src/codes.ts:592` | Split `pharmacy.dispense.create` into its module for grouping in the UI. |
...
| `signsOffRulePacks` <sub>local</sub> | `(p: PermissionCode): boolean` | `packages/permissions/src/roles.ts:89` |  |

## const

| name | signature | at | notes |
| --- | --- | --- | --- |
| `ALL_PERMISSIONS` | `Object.values(PERMISSIONS) as PermissionCode[]` | `packages/permissions/src/codes.ts:589` |  |
...
| `REGULATORY_SIGN_OFF` <sub>local</sub> | `: PermissionCode[]` | `packages/permissions/src/roles.ts:87` |  |
| `SYSTEM_ROLE_DEFINITIONS` | `: SystemRoleDefinition[]` | `packages/permissions/src/roles.ts:111` |  |
...
| `PermissionCode` | `(typeof PERMISSIONS)[keyof typeof PERMISSIONS]` | `packages/permissions/src/codes.ts:587` |  |
