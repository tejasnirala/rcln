 ...
 *
 * ⚠️ THIS EXISTS SO DISPENSING CAN PLAN AND POST AGAINST ONE SNAPSHOT (PI-7).
 *   The same reasoning `evaluateWithin` gives: the dispense service has already
 *   locked buckets and is about to write the ledger, and calling the wrapper
 *   below there would open a SECOND transaction — which cannot see the
 *   uncommitted work of the first, takes its own snapshot, and on a pooled
 *   connection can deadlock against the locks the outer one holds. A plan read
 *   outside the transaction that acts on it is also a plan about a different
 *   moment than the write.
 */
export async function planStockAllocationWithin(
  tx: TxClient,
  ctx: TenantContext,
...
/** The same plan, for a caller that is not already in a transaction. */
export async function planStockAllocation(
  ctx: TenantContext,
  input: AllocationPlanRequest
): Promise<AllocationPlanResponse> {
  return withTenant(ctx, async (tx) => planStockAllocationWithin(tx, ctx, input));
}
