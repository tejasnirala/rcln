...
import type { BranchSummary, GoodsReceiptListResponse, SupplierSummary } from '@rcln/contracts';
 ...
 * Every delivery.
 *
 * ⚠️ "WAITING FOR INSPECTION" IS A LINE-LEVEL QUESTION SHOWN AS A DOCUMENT FILTER. A
 *   delivery is partly accepted and partly pending all the time — inspection happens
 *   line by line — so the API asks it as an EXISTS over the lines rather than a flag
 *   on the header that some code path has to remember to maintain.
 */
const STATUSES = [
  { value: '', label: 'Any status' },
  { value: 'DRAFT', label: 'Draft' },
  { value: 'POSTED', label: 'On the shelf' },
...
interface Props {
  goodsReceipts: GoodsReceiptListResponse['goodsReceipts'];
...
export function GoodsReceiptList({ goodsReceipts, meta, branches, suppliers, canManage }: Props) {
  ...
  const awaiting = searchParams.get('awaitingQuality') === 'true';
    ...
    <div className="space-y-8">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-muted mt-1 text-[0.875rem]">
            What has turned up, what it cost, and what is waiting to be checked.
          </p>
        </div>
        {canManage ? (
          <Link
      ...
      <div className="border-rule bg-card grid gap-3 rounded-md border p-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Select
          ...
          options={STATUSES}
          onChange={(event) => setFilter({ status: event.target.value, awaitingQuality: '' })}
        />
        <div className="flex items-end">
          <Button
            type="button"
            variant={awaiting ? 'primary' : 'secondary'}
            onClick={() => setFilter({ awaitingQuality: awaiting ? '' : 'true', status: '' })}
          >
            {awaiting ? 'Showing what needs checking' : 'Needs checking'}
          </Button>
        </div>
      </div>
        ...
        {goodsReceipts.length === 0 ? (
          <div className="border-rule bg-card rounded-md border p-10 text-center">
            <p className="text-ink text-[0.9375rem]">
              {awaiting ? 'Nothing is waiting to be checked.' : 'No deliveries recorded.'}
            </p>
            <p className="text-muted mx-auto mt-2 max-w-md text-[0.875rem]">
              {awaiting
                ? 'Stock is held for inspection only where the branch is set up to inspect deliveries.'
                : 'Record one when boxes arrive. Posting it is what puts the stock on a shelf.'}
            </p>
          </div>
        ) : (
          <ul className="border-rule divide-rule bg-card divide-y rounded-md border">
            {goodsReceipts.map((receipt) => (
              <li key={receipt.id} className="px-4 py-3.5">
                <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                  ...
                  <span className="text-muted text-[0.8125rem]">{receipt.supplierName}</span>
                  ...
                  {receipt.awaitingQualityLineCount > 0 ? (
                    <span className="text-signal border-signal/30 rounded-xs border px-1.5 py-0.5 text-[0.6875rem]">
                      {receipt.awaitingQualityLineCount} to check
                    </span>
                  ) : null}
                  <span className="text-ink ml-auto font-mono text-[0.875rem]">
                    {receipt.currency} {(receipt.totalMinor / 100).toFixed(2)}
                  </span>
                </div>
                <p className="text-muted mt-1 text-[0.8125rem]">
                  {receipt.branchName} · {receipt.locationName} · {receipt.receivedAt.slice(0, 10)}
                  {receipt.purchaseOrderNumber ? ` · against ${receipt.purchaseOrderNumber}` : ''}
