...
import {
  buildUnitGraph,
  sweepExpiredStock as sweepBranch,
  type MovementDeps,
  type SweepResult,
  type UnitGraph,
} from '@rcln/inventory';
...
/**
 * The worker's own binding of the movement engine's dependencies.
 *
 * ⚠️ IT IS NOT THE API'S. The API injects `recordAudit` from its audit service
 *   and `loadUnitGraph` from its product service; the worker cannot import
 *   either — they live in an application. These two are the same reads and the
 *   same write, expressed against `@rcln/db` directly, and they must stay in
 *   step with their API counterparts.
 *
