...
const KARNATAKA: SupplierRegistration = {
  countryCode: 'IN',
  regionCode: 'KA',
  scheme: 'GST',
...
const IRELAND: SupplierRegistration = {
  countryCode: 'IE',
  regionCode: null,
  scheme: 'VAT',
...
describe('choosing the registration', () => {
  it('prefers a state registration over a country-wide one', () => {
    const countryWide: SupplierRegistration = {
      ...KARNATAKA,
      regionCode: null,
  ...
  it('builds the place of supply from country and region', () => {
    expect(placeOfSupplyFor(customer({ regionCode: 'KA' }))).toBe('IN-KA');
    expect(placeOfSupplyFor(customer({ countryCode: 'IE', regionCode: null }))).toBe('IE');
  });
});
