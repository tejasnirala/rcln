-- =============================================================================
-- Fee schedule, doctor compensation, and the reschedule trail.
--
-- ⚠️ THE ORDER IN THIS FILE IS LOAD-BEARING. `fee_schedule_entries` is created,
--   then the two fee columns on `doctor_branch_settings` are COPIED INTO IT, and
--   only then dropped. Reordering the drop above the copy loses every price
--   every clinic has set, silently and irreversibly — the columns are the only
--   thing pricing a consultation today.
--
-- Hand-assembled from `prisma migrate diff` because that command cannot see the
-- data migration, the NULLS NOT DISTINCT index, or the RLS policies.
-- =============================================================================

-- CreateEnum
CREATE TYPE "PayoutInterval" AS ENUM ('MONTHLY', 'FORTNIGHTLY', 'WEEKLY', 'DAILY', 'HOURLY', 'PER_SESSION');

-- CreateEnum
CREATE TYPE "RescheduleInitiator" AS ENUM ('PATIENT', 'CLINIC');

-- AlterTable
ALTER TABLE "appointments" ADD COLUMN     "booked_fee" DECIMAL(14,2);
...
CREATE TABLE "fee_schedule_entries" (
    ...
    CONSTRAINT "fee_schedule_entries_pkey" PRIMARY KEY ("id")
);
...
CREATE TABLE "doctor_compensation" (
    ...
    CONSTRAINT "doctor_compensation_pkey" PRIMARY KEY ("id")
);
...
CREATE TABLE "appointment_reschedules" (
    ...
    "from_start" TIMESTAMPTZ(6) NOT NULL,
    "to_start" TIMESTAMPTZ(6) NOT NULL,
    "from_doctor_profile_id" UUID NOT NULL,
...
-- CreateIndex
CREATE INDEX "fee_schedule_entries_organization_id_fee_type_idx" ON "fee_schedule_entries"("organization_id", "fee_type");

-- CreateIndex
CREATE UNIQUE INDEX "doctor_compensation_organization_id_doctor_profile_id_key" ON "doctor_compensation"("organization_id", "doctor_profile_id");

-- CreateIndex
CREATE INDEX "appointment_reschedules_organization_id_appointment_id_crea_idx" ON "appointment_reschedules"("organization_id", "appointment_id", "created_at");

-- AddForeignKey
ALTER TABLE "fee_schedule_entries" ADD CONSTRAINT "fee_schedule_entries_organization_id_fkey" FOREIGN KEY ("organization_id") REFERENCES "organizations"("id") ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "fee_schedule_entries" ADD CONSTRAINT "fee_schedule_entries_organization_id_branch_id_fkey" FOREIGN KEY ("organization_id", "branch_id") REFERENCES "branches"("organization_id", "id") ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "fee_schedule_entries" ADD CONSTRAINT "fee_schedule_entries_organization_id_doctor_profile_id_fkey" FOREIGN KEY ("organization_id", "doctor_profile_id") REFERENCES "doctor_profiles"("organization_id", "id") ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "doctor_compensation" ADD CONSTRAINT "doctor_compensation_organization_id_fkey" FOREIGN KEY ("organization_id") REFERENCES "organizations"("id") ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "doctor_compensation" ADD CONSTRAINT "doctor_compensation_organization_id_doctor_profile_id_fkey" FOREIGN KEY ("organization_id", "doctor_profile_id") REFERENCES "doctor_profiles"("organization_id", "id") ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "appointment_reschedules" ADD CONSTRAINT "appointment_reschedules_organization_id_fkey" FOREIGN KEY ("organization_id") REFERENCES "organizations"("id") ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "appointment_reschedules" ADD CONSTRAINT "appointment_reschedules_organization_id_branch_id_fkey" FOREIGN KEY ("organization_id", "branch_id") REFERENCES "branches"("organization_id", "id") ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "appointment_reschedules" ADD CONSTRAINT "appointment_reschedules_organization_id_appointment_id_fkey" FOREIGN KEY ("organization_id", "appointment_id") REFERENCES "appointments"("organization_id", "id") ON DELETE CASCADE ON UPDATE CASCADE;

-- ---------------------------------------------------------------------------
-- ⚠️ THE UNIQUE KEY, WITH `NULLS NOT DISTINCT`, REPLACING PRISMA'S.
--
-- Prisma emits a plain unique index, and a plain unique index does not
-- constrain NULLs — every row with a NULL `doctor_profile_id` or `branch_id`
-- would be unique against every other by definition. Both columns are nullable
-- BY DESIGN here (NULL = "all doctors" / "all branches"), so without this a
-- clinic can hold two different organization-wide `NEW` prices and which one a
-- patient pays is whichever the planner returns first. ADR-0004.
--
-- No partial `WHERE deleted_at IS NULL` clause: this table has no soft delete.
-- Removing a price means the level below it applies again, which is a real
-- edit rather than a tombstone, and a soft-deleted row would keep occupying
-- the key it can no longer justify.
-- ---------------------------------------------------------------------------
CREATE UNIQUE INDEX "fee_schedule_entries_scope_key"
  ON "fee_schedule_entries" ("organization_id", "doctor_profile_id", "branch_id", "fee_type")
  NULLS NOT DISTINCT;

-- ---------------------------------------------------------------------------
-- ⚠️ THE DATA MIGRATION. RUN BEFORE THE DROP BELOW, NEVER AFTER.
--
-- `doctor_branch_settings.consultation_fee` and `.follow_up_fee` are the only
-- thing pricing a consultation in this product today. `resolveCharge()` is the
-- only thing that PRICES from them; `doctor.service.ts` reads them for the
-- profile and writes them from `PUT /doctors/:id/branch-settings`, and the web
-- profile displays them. All four move to the fee schedule in this station.
--
-- They become entries scoped to (doctor, branch) — the most specific level —
-- because that is exactly what they meant: this doctor's price at this branch.
--
-- `NEW` and `FOLLOW_UP` are the `AppointmentVisitType` members they correspond
-- to. WALK_IN, TELECONSULT and PROCEDURE had no column and therefore have no
-- row: those visit types were being billed as consultations, and after this
-- they are honestly unpriced until the clinic sets them. That is a visible
-- change in behaviour and the right one — a teleconsult billed at the
-- in-person rate because nothing could tell them apart is a wrong bill that
-- looks correct.
--
-- NULL fees are skipped rather than written as zero: a blank column meant "not
-- set", and zero means free (see the model note). Writing 0 here would turn
-- every unconfigured doctor into a free one.
-- ---------------------------------------------------------------------------
INSERT INTO "fee_schedule_entries"
  ("id", "organization_id", "branch_id", "doctor_profile_id", "fee_type", "amount", "created_at", "updated_at")
SELECT
  gen_random_uuid(),
  s."organization_id",
  s."branch_id",
  ...
  now(),
  now()
FROM "doctor_branch_settings" s
WHERE s."consultation_fee" IS NOT NULL;
...
SELECT
  ...
  now()
FROM "doctor_branch_settings" s
WHERE s."follow_up_fee" IS NOT NULL;

-- ---------------------------------------------------------------------------
-- Only now. `follow_up_free_days` stays: it is a window, not a price.
-- ---------------------------------------------------------------------------
ALTER TABLE "doctor_branch_settings" DROP COLUMN "consultation_fee",
DROP COLUMN "follow_up_fee";

-- ---------------------------------------------------------------------------
-- Naming drift from earlier hand-written SQL, which `migrate diff` reports on
-- every run until it is applied. Renames only — no data and no behaviour moves.
-- ---------------------------------------------------------------------------
ALTER TABLE "appointment_vitals" RENAME CONSTRAINT "appointment_vitals_revision_of_fkey" TO "appointment_vitals_organization_id_revision_of_id_fkey";
ALTER INDEX "appointment_vitals_organization_id_revision_of_id_supersed_idx" RENAME TO "appointment_vitals_organization_id_revision_of_id_supersede_idx";

-- =============================================================================
-- RLS. The full contents of prisma/rls/enable-rls.sql, appended as CONVENTIONS
-- requires: the file is idempotent (DROP POLICY IF EXISTS then CREATE), so
-- re-running it re-states every table's policy rather than only the new ones.
-- Three tenant tables are added here, taking db:rls:check from 48 to 51.
-- =============================================================================
-- =============================================================================
...
DECLARE
  ...
  org_scoped text[] := ARRAY[
    ...
    -- untaxed. None of that reasoning transfers to these two, whose rows belong
    -- to the very organization reading them — so they get an ordinary policy,
    -- and without one a clinic reads another clinic's GSTIN and rate card.
