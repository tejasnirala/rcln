 ...
 *
 * ⚠️ THIS FILE LIVES IN A PACKAGE RATHER THAN IN `apps/api` BECAUSE THE EXPIRY
 *   SWEEP IS A WORKER PROCESSOR. The worker cannot import from the API — they
 *   are two applications — and every quantity the sweep writes has to go through
 *   this one engine and the one ledger writer beside it. `apps/api/src/services/
 *   product/units.ts` is now a re-export, so no existing import changed.
 *
...
export function rational(n: bigint, d: bigint): Rational {
  if (d === 0n) {
    ...
    throw invalid('A unit conversion cannot have a zero denominator.');
  }
...
export function invert(a: Rational): Rational {
  if (a.n === 0n) {
    throw invalid('A unit conversion cannot have a zero ratio.');
  }
 ...
 *
 * ⚠️ NEVER `Number(a) - Number(b)`. This is the comparison the ledger's
 *   sufficiency check runs — "does the shelf hold at least what is being taken"
 *   — and at eighteen digits a double has already lost the tail before the
 *   subtraction happens. Two quantities that differ by a millionth of a tablet
 *   compare equal, the check passes, and the balance goes negative into a CHECK
 *   constraint instead of into a clean 409.
 *
 * Cross-multiplied over the rationals, so nothing is divided and nothing is
 * rounded. Both denominators are positive by construction (`rational` normalises
 * the sign onto the numerator), so the inequality does not flip.
 */
...
export function conversionFactor(graph: UnitGraph, fromUnitId: string, toUnitId: string): Rational {
  ...
  if (!from) throw invalid('The source unit is not available to this clinic.');
  if (!to) throw invalid('The target unit is not available to this clinic.');
...
export function assertValidPackaging(levels: PackagingLevelRow[]): void {
  ...
  for (const row of sorted) {
    if (row.level < 0) throw invalid('A packaging level cannot be negative.');
    if (seen.has(row.level)) {
      throw invalid(`This product has two packaging rows at level ${row.level}.`);
    }
    seen.add(row.level);
  ...
  if (sorted[0]?.level !== 0) {
    throw invalid('A packaging hierarchy must start at level 0, which is the product’s base unit.');
  }
  ...
  if (units.size !== sorted.length) {
    throw invalid('The same unit is used for two packaging levels.');
  }
}
